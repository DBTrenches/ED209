...
10,$here\..\sql\tables\Logs.sql,tables,Logs
20,$here\..\sql\tables\SpidHallPass.sql,tables,SpidHallPass
30,$here\..\sql\tables\Config.sql,tables,Config
40,$here\..\sql\tables\ConfigSetting.sql,tables,ConfigSetting
48,$here\..\sql\views\all_request_usage.sql,views,all_request_usage
49,$here\..\sql\views\all_query_usage.sql,views,all_query_usage
50,$here\..\sql\views\LongRunningQueries.sql,views,LongRunningQueries
60,$here\..\sql\procedures\Enforcer.sql,procedures,Enforcer
70,$here\..\sql\procedures\RequestHallPass.sql,procedures,RequestHallPass
80,$here\..\sql\procedures\KillLongRunningQueries.sql,procedures,KillLongRunningQueries
"@ | ConvertFrom-Csv
...
if($deployConfig -eq "Y"){
    ...
    if(($config.Count -gt 0) -and ($configSetting.Count -gt 0)){
        ...
        $configSetting | % {
            ...
            $s1 = $_.Setting1
            $s2 = $_.Setting2
            Invoke-Sqlcmd @conn -Query "Insert ED209.ConfigSetting(ConfigID,Setting1,Setting2) values ($id,nullif('$s1',''),nullif('$s2',''));"
        }
    }
}
